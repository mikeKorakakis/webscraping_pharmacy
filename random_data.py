...
def create_categories(categories_data, placeholder_dir):
    ...
    for category in categories_data:
        ...
        # )
        image_name='categories/' + category["fields"]["slug"] + ".png"
...
def create_collections(data, placeholder_dir):
    ...
    for collection in data:
        ...
        image_name = COLLECTION_IMAGES[1]
        background_image = get_image(placeholder_dir, image_name)
        defaults["background_image"] = background_image
...
def create_products(products_data, placeholder_dir, create_images,images_mapping):
    for product in products_data:
        ...
        #     # images = IMAGES_MAPPING.get(pk, [])
            ...
            for image_name in images:
                print(image_name)
                create_product_image(product, placeholder_dir, image_name,images_mapping)
...
def create_products_by_schema(placeholder_dir, create_images):
    ...
    for item in db_items:
        ...
        if(model ==  'product.product'):
            pk=item['pk']
            images = item['fields'].pop("images")
            images_mapping[pk]=images
...
def create_product_image(product, placeholder_dir, image_name, images_mapping):
...
def get_image(image_dir, image_name):
    ...
    img_path_placeholder = os.path.join(image_dir, "categories/placeholder.png")
    if os.path.isfile(img_path):
      return File(open(img_path, "rb"), name=image_name)
    else:
      return File(open(img_path_placeholder, "rb"), name=img_path_placeholder)
